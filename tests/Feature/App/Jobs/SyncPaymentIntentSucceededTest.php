...
it('throws when no analysis request matches the payment intent', function (): void {
    ...
    $handleJob = fn () => $job->handle();

    expect($handleJob)
        ->toThrow(
            \RuntimeException::class,
            'analysis request not found for payment_intent (yet)'
        );
});
...
it('marks pending request paid, dispatches analysis job, and sets dedupe cache', function (): void {
    ...
    Queue::assertPushed(
        ...
        fn ($job) => $job->analysisRequestId === $request->id
    );
...
it('dispatches analysis when already paid and queued but dedupe key is absent', function (): void {
    ...
    $hasDedupeKey = Cache::has('stripe:process-analysis-dispatched:pi_sync_paid_queued_1');
    expect($hasDedupeKey)
        ->toBeTrue();
});
...
it('does not dispatch again when paid and queued and dedupe key exists', function (): void {
    ...
    $expiresAt = now()
                    ->addMinutes(30);
    Cache::put(
        'stripe:process-analysis-dispatched:pi_sync_deduped_1',
        true,
        $expiresAt
    );
...
it('logs warning on failed', function (): void {
    ...
    Log::shouldHaveReceived('warning')
        ->with(
            ...
            \Mockery::on(
                fn (array $context): bool => $context['payment_intent_id'] === 'pi_sync_fail_1'
