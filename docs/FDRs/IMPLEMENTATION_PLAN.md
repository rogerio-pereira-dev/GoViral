...
- Add migration for `discount_coupons`: id (UUID), code (string, unique), value (integer 0–100), expires_at (nullable timestamp), max_uses (nullable integer), times_used (integer, default 0), timestamps. Model with fillable, casts, and scopes (e.g. valid for checkout).
- Add migration to alter `analysis_requests`: nullable `discount_coupon_id` (foreign key to `discount_coupons.id`). Update AnalysisRequest model (fillable, relationship to DiscountCoupon).
- Add CRUD routes under `/core/*` (e.g. `/core/coupons` or `/core/discount-coupons`), protected by auth (and verified if core requires it). Controller(s) for index, create, store, edit, update, destroy; Form Requests for validation (code unique on create, value 0–100, expiration type and related fields). Authorization: only authenticated users; prevent unauthenticated or forged creation/update (abuse prevention). Admin delete must not hard-delete (use soft delete e.g. `deleted_at` or inactive flag).
- Build core UI (Inertia/Vue) to list, create, edit, delete coupons; expiration type (never / after X days / after X uses) and related inputs; delete = soft delete or hide (no hard delete).
- Checkout at `/start-growth`: add optional coupon code input; backend endpoint to validate code (exists, not expired, not exhausted) and return discounted amount; apply discount to Stripe payment amount. Store coupon id on analysis_requests (e.g. `discount_coupon_id`) so webhook can increment usage and the record keeps the reference.
- Webhook (payment confirmation): when payment succeeded and coupon was used, increment coupon `times_used` (for all coupon types). Do not delete the coupon; coupons are never hard-deleted.
- No scheduler that deletes coupon rows; expired or exhausted coupons remain in DB and are only invalid for new uses at checkout.
- Tests: Feature tests for CRUD (auth required, validation, soft delete); checkout validation and discount application; webhook increments times_used and analysis_requests has discount_coupon_id set; no hard delete of coupons.
