...
it('rejects request without valid signature with 403', function (): void {
    $stripeWebhookRoute = route('stripe.webhook');
    $payload = [
...
it('returns 400 when body is not valid JSON', function (): void {
    $body   = 'not valid json';
    $config = config('cashier.webhook.secret');
    $header = stripeWebhookSignature($body, $config);
    $route = route('stripe.webhook');
...
it('dispatches a sync job when payment_intent.succeeded has no matching analysis request yet', function (): void {
    ...
    Queue::assertPushed(
            SyncPaymentIntentSucceeded::class,
            fn ($job) => $job->paymentIntentId === 'pi_live_race_1'
        );
    Queue::assertNotPushed(ProcessAnalysisRequest::class);
});
...
it('updates record and dispatches job on payment_intent.succeeded', function (): void {
    ...
    expect($analysisRequest->payment_status)
        ->toBe('paid')
        ->and($analysisRequest->processing_status)
        ->toBe('queued');

    Queue::assertPushed(
            ProcessAnalysisRequest::class,
            fn ($job) => $job->analysisRequestId === $analysisRequest->id
        );
});
...
it('increments coupon times_used when analysis request has discount_coupon_id', function (): void {
    ...
    $timesUsed = $coupon->fresh()->times_used;
    expect($timesUsed)
        ->toBe(1);
});
