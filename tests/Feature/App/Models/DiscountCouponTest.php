...
it('relates analysis requests to discount coupon', function (): void {
    ...
    $request = AnalysisRequest::factory()
                    ->create([
...
it('computes discountedAmountCents with percentage and minimum amount', function (): void {
    ...
    expect($amountWithOneHundredPercentDiscount)
        ->toBe(50);
});
...
it('findValidByCode returns null for blank code', function (): void {
    ...
    expect($couponWithEmptyCode)
        ->toBeNull();
    expect($couponWithBlankCode)
        ->toBeNull();
});
...
it('treats expires_at as date-only and keeps coupons valid through expiration date', function (): void {
    ...
    $couponToday = DiscountCoupon::factory()
                        ->create([
                            'expires_at' => $today,
                            'max_uses' => null,
                            'times_used' => 0,
    ...
    $tomorrow = now()
                    ->addDay()
                    ->toDateString();
    $couponTomorrow = DiscountCoupon::factory()
                        ->create([
                            'expires_at' => $tomorrow,
                            'max_uses' => null,
                            'times_used' => 0,
    ...
    $yesterday = now()
                    ->subDay()
                    ->toDateString();
    $couponYesterday = DiscountCoupon::factory()
                            ->create([
    ...
    expect($checkoutIncludesToday)
        ->toBeTrue();
    expect($checkoutIncludesTomorrow)
        ->toBeTrue();
    expect($checkoutIncludesYesterday)
        ->toBeFalse();
});
